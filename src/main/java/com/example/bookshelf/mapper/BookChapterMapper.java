...
public interface BookChapterMapper {
     ...
     * 根据书籍和章节序号查询章节是否存在
     */
    @Select("SELECT COUNT(*) FROM book_chapter WHERE book_id = #{bookId} AND chapter_order = #{chapter_order}")
    Integer findCountByBookIdByChapterOrder(@Param("bookId") Long bookId,@Param("chapter_order") Long chapter_order);
